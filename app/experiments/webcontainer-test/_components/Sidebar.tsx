...
import { WorkflowNode, WorkflowNodeData } from "../_state";
...
interface SidebarProps {
  ...
  contracts: Pick<WorkflowNodeData, "id" | "title" | "flow" | "spec">[];
}

export function Sidebar({ nodes, edges, output, activeLayer, contracts }: SidebarProps) {
  return (
    <div className="w-[400px] border-l bg-background">
      <Tabs defaultValue="console" className="flex h-full flex-col">
        ...
        <TabsContent value="context" className="flex-grow overflow-auto p-4 text-sm">
          <div className="mb-4">
          ...
          <div className="space-y-4">
            {contracts.map((contract) => (
              <div key={contract.id} className="rounded-2xl border border-slate-200 bg-white p-3">
